...
export async function Loginfetch(email, password) {
  ...
  export async function Logout(accessToken) {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/member/logout`, {
      cache: 'no-store',
      method: "POST",
      headers: { 
  ...
  export async function KakaoLogout() {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/kakao/logout`, {
      cache: 'no-store',
      method: "POST",
      headers: { 
        "Content-Type": "application/json",
...
export async function fetchUserEmail(nick_name) {
  try {
   const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/member/email?nick_name=${nick_name}`, {
      cache: 'no-store',
    });
    const data = await response.json();
    console.log(data);
    ...
    console.error('사용자 이메일 정보를 가져오는 중 오류가 발생했습니다.', error);
    throw error;
  }
};
